...
#(
  ...
  // PLRU replacement signals
  logic [CVA6Cfg.ICACHE_SET_ASSOC_WIDTH-1:0] plru_replace_way; // PLRU victim way
  logic [CVA6Cfg.ICACHE_SET_ASSOC_WIDTH-1:0] plru_access_way; // way being accessed
  ...
  // PLRU: input mux — hit_idx khi hit, repl_way_oh_q(binary) khi fill
  assign plru_access_way = cache_wren ? repl_way : hit_idx;
  ...
  // Tree-PLRU replacement — thay thế LFSR
  plru #(
    .NUM_WAYS (CVA6Cfg.ICACHE_SET_ASSOC)   // FIX: WAYS → NUM_WAYS
  ) i_plru (
    .clk_i         (clk_i),
    .rst_ni        (rst_ni),
    .flush_i       (flush_i),              // FIX: thêm flush_i
    .update_i      ((cmp_en_q & |cl_hit) | cache_wren),
